// Stock Zero - Recipes Module TypeScript

import { Recipe, InventoryItem, ApiResponse } from './types';
...
class RecipeManager {
    ...
    private async updateRecipeCosts(): Promise<void> {
        // Update cached recipes with current ingredient costs
    ...
    async loadRecipes(): Promise<Recipe[]> {
        try {
            ...
            // Cache recipes for quick access
    ...
    async addRecipe(recipeData: Omit<Recipe, 'id'>): Promise<ApiResponse<Recipe>> {
        try {
            ...
            // Calculate recipe costs and margins
    ...
    async updateRecipe(id: string, updates: Partial<Recipe>): Promise<ApiResponse<Recipe>> {
        try {
            // If ingredients or selling price changed, recalculate costs
    ...
    async calculateRecipeCosts(recipe: Omit<Recipe, 'id'>): Promise<RecipeCalculation> {
        try {
            ...
            for (const ingredient of recipe.ingredients) {
                ...
                if (inventoryItem) {
                    // Calculate cost based on inventory item cost
                    const unitConversion = this.convertUnits(ingredient.quantity, ingredient.unit, inventoryItem.unit);
                    ingredientCost = (unitConversion / parseFloat(inventoryItem.unit as any)) * inventoryItem.cost;
                } else {
                    // Estimate cost based on market prices
    ...
    private async estimateIngredientCost(name: string, quantity: number, unit: string): Promise<number> {
        // Simplified cost estimation based on ingredient type
        const marketPrices: Record<string, number> = {
            'harina': 2.5, // per kg
            'azúcar': 3.0, // per kg
            'huevo': 0.2, // per unit
            'leche': 1.5, // per liter
            'mantequilla': 8.0, // per kg
            'sal': 1.0, // per kg
            'levadura': 15.0, // per kg
            'vainilla': 20.0, // per liter
            'chocolate': 12.0, // per kg
            'fruta': 5.0, // per kg
            'nuez': 25.0, // per kg
            'canela': 8.0, // per kg
            'aceite': 4.0, // per liter
            'agua': 0.001 // per liter
        };

        const normalizedName = name.toLowerCase();
        const basePrice = marketPrices[normalizedName] || 5.0; // Default price

        // Convert to standard unit (kg or liter)
    ...
    private convertToStandardUnit(quantity: number, unit: string): number {
        // Convert to kg for solids, liters for liquids
        return this.convertUnits(quantity, unit, 'kg');
    }

    // Recipe suggestions based on available inventory
    async getRecipeSuggestions(): Promise<RecipeSuggestion[]> {
        try {
    ...
    private analyzeRecipeFeasibility(recipe: Recipe, inventory: InventoryItem[]): {
        score: number;
        reasons: string[];
    } {
        let score = 0;
        ...
        for (const ingredient of recipe.ingredients) {
            ...
            if (inventoryItem) {
                ...
                if (inventoryItem.quantity >= unitConversion) {
                    ...
                    score += 20; // Full points for available ingredient
                } else {
                    score += 5; // Partial points for ingredient in inventory
    ...
    // Recipe scaling
    async scaleRecipe(recipeId: string, targetYield: number): Promise<ApiResponse<Recipe>> {
        try {
    ...
    // Cost analysis for menu pricing
    async analyzeMenuPricing(): Promise<{
        recipes: Array<{
            recipe: Recipe;
        ...
        try {
            ...
            for (const recipe of recipes) {
                ...
                // Suggest pricing based on cost and market factors
                ...
                if (currentProfit < 30) underpricedCount++;
                if (currentProfit > 70) overpricedCount++;
            }
    ...
    private suggestOptimalPrice(costPerUnit: number): number {
        // Suggest price with 50% profit margin as baseline
    ...
    private calculatePriceRange(costPerUnit: number): { min: number; max: number; optimal: number } {
        const minMargin = 0.3; // 30% minimum
        const maxMargin = 0.7; // 70% maximum
        const optimalMargin = 0.5; // 50% optimal
    ...
    private roundToAttractivePrice(price: number): number {
        // Round to nearest 0.50 or 0.99
        const rounded = Math.ceil(price);
        if (rounded - price < 0.5) {
            return rounded - 0.01; // .99 pricing
        } else {
            return rounded - 0.5; // .50 pricing
        }
    }

    // Load recipe data for UI
    async loadRecipesList(): Promise<void> {
        try {
            const recipes = await this.loadRecipes();
    ...
    async loadRecipeSuggestions(): Promise<void> {
        try {
    ...
    editRecipe(recipeId: string): void {
        ...
        if (recipe) {
            ...
            const editForm = document.getElementById('recipeEditForm') as HTMLFormElement;
            if (editForm) {
                // Fill form fields
                (editForm.elements.namedItem('recipeName') as HTMLInputElement).value = recipe.name;
                (editForm.elements.namedItem('recipeCategory') as HTMLSelectElement).value = recipe.category;
                (editForm.elements.namedItem('recipeYield') as HTMLInputElement).value = recipe.yield.toString();
                (editForm.elements.namedItem('recipePrice') as HTMLInputElement).value = recipe.sellingPrice.toString();
