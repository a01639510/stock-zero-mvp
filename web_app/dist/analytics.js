...
class AnalyticsManager {
    ...
    updateKPICards(sales, inventory, recipes) {
    ...
    async loadDashboardCharts(sales, inventory) {
        try {
    ...
    async createSalesTrendChart(sales, containerId) {
    ...
    async createSalesByCategoryChart(sales, containerId) {
        const container = document.getElementById(containerId);
        if (!container)
            return;
    ...
    async createInventoryStatusChart(inventory, containerId) {
        const container = document.getElementById(containerId);
        if (!container)
            return;
    ...
    async createInventoryByCategoryChart(inventory, containerId) {
        const container = document.getElementById(containerId);
        if (!container)
            return;
    ...
    async createRecipeProfitabilityChart(recipes, containerId) {
        const container = document.getElementById(containerId);
        if (!container)
            return;
        ...
        const chartData = {
            data: [{
                    ...
                    type: 'bar',
                    marker: {
                        color: sortedRecipes.map(recipe => recipe.profitMargin > 50 ? this.colors.success :
    ...
    async createTopProductsChart(sales, containerId) {
        const container = document.getElementById(containerId);
        if (!container)
            return;
    ...
    groupSalesByDate(sales, groupBy = 'day') {
        const grouped = {};
        sales.forEach(sale => {
    ...
    categorizeInventoryStatus(inventory) {
    ...
    refreshInventoryAnalytics() {
        this.loadInventoryAnalytics();
    }
    refreshRecipeAnalytics() {
