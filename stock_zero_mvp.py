...
try:
    ...
    ultimos_30 = df_ventas[df_ventas['fecha'] >= ultimo_dia - timedelta(days=30)]
    ventas_diarias = ultimos_30.groupby('fecha')['cantidad_vendida'].sum().sort_index()
...
with col4:
    costo = df_stock.get('costo_unitario', pd.Series([10] * len(df_stock)))
